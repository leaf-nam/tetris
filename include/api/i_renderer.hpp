...
public:
     ...
     * @param 현재시간(초)
     */
    virtual void render_timer(const int sec) = 0;

    /**
     * @brief 다음 블럭 3개 렌더링
     * @param 다음 블럭 3개를 가진 배열 포인터(순서 중요)
     */
    virtual void render_next_block(const Tetromino* tetrominoArray) = 0;
     ...
     * @param 현재 점수
     */
    virtual void render_score(const int score) = 0;

    /**
     * @brief 레벨 렌더링
     * @param 현재 레벨
     */
    virtual void render_level(const int level) = 0;
