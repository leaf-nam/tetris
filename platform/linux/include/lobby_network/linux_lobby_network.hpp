#ifndef __LINUX_LOBBY_NETWORK_HPP__
#define __LINUX_LOBBY_NETWORK_HPP__
...
class LinuxLobbyNetwork : public ILobbyNetwork
{
  private:
    int sock;
    int epfd;
  ...
  public:
    ...
    void send_udp(const char* room_master_id, std::unordered_map<std::string, std::string> ids_ips,
                  ...
                  int is_update, int is_broadcast_delete, int is_chat, 
    ...
    void send_multi_udp(const char* room_master_id,
                        ...
                        const char* room_name, int id_len,
                        int is_enter_not_success, int is_game_start, int is_broadcast, int is_update, int is_broadcast_delete,
