...
static const unordered_map<char, string> ASCII_ART = {{'T',
                                                       R"(
████████╗
╚══██╔══╝
   ██║   
...
static unordered_map<int, string> color_map = {
    {MinoType::I, "\x1b[38;2;139;233;253m"}, {MinoType::O, "\x1b[38;2;80;250;123m"},
    {MinoType::Z, "\x1b[38;2;255;184;108m"}, {MinoType::S, "\x1b[38;2;255;121;198m"},
    {MinoType::J, "\x1b[38;2;189;147;249m"}, {MinoType::L, "\x1b[38;2;255;85;85m"},
    {MinoType::T, "\x1b[38;2;241;250;140m"}, {MinoType::OBSTACLE, "\x1b[38;2;98;114;164m"},
};
...
LinuxRender::~LinuxRender() { restore_window(); }

void LinuxRender::go(int row, int col) { cout << "\x1b[" << row + 1 << ";" << col + 1 << "H"; }

void LinuxRender::render_mino(int row, int col, const Mino& tetromino, int type)
{
    int wr = row, wc = col;
    ...
    for (int r = 0; r < MINO_SIZE; ++r) {
        ...
        for (int c = 0; c < MINO_SIZE; ++c) {
            cout << ((tetromino[r][c] != 0) ? color + "[]" : "  ") << ansi_reset;
        }
    }
}
 ...
 */
void LinuxRender::render_background()
{
    int wr = 0, wc = 0;
    vector<vector<string>> lines;

    for (char c : TITLE) {
        const string& art = ASCII_ART.at(c);
        stringstream ss(art);
        string line;
        vector<string> v;
    ...
    for (size_t row = 0; row < lines[0].size(); ++row) {
        ...
        for (size_t i = 0; i < TITLE.size(); ++i) {
            ...
            if (!s.empty() && s.back() == '\n')
                ...
                cout << color_map[i % 7] << s << ansi_reset;
 ...
 */
void LinuxRender::render_board(const Board& board, const Tetromino& tetromino)
{
    string color;
    ...
    if (board.has_active_mino()) {
        ...
        for (int r = BOARD_UPPER; r < BOARD_ROW; ++r) {
            ...
            for (int c = 0; c < BOARD_COL; ++c) {
                mino_c = c - pos_c;
                if (mino_r >= 0 && mino_r < MINO_SIZE && mino_c >= 0 && mino_c < MINO_SIZE)
                    cout << ((m[mino_r][mino_c] != 0) ? color + "[]" : "\x1b[2C") << ansi_reset;
                else
                    cout << "\x1b[2C";
            }
        }
 ...
 * @param 현재시간(초)
 */
void LinuxRender::render_timer(int sec)
{
    int wr = 19, wc = 32;
 ...
 */
void LinuxRender::render_next_block(const int* tetrominoArray)
{
    int wr = 7, wc = 0;
    ...
    // next mino
    render_mino(wr + 1, wc + 1, m1, tetrominoArray[0]);

    render_mino(wr + 5, wc + 1, m2, tetrominoArray[1]);

    render_mino(wr + 9, wc + 1, m3, tetrominoArray[2]);
 ...
 * @param 홀드할 블럭
 */
void LinuxRender::render_hold(const Tetromino& tetromino)
{
    int wr = 7, wc = 32;
    ...
    if (saved_mino_type != -1) {
        ...
        render_mino(wr + 1, wc + 1, m, saved_mino_type);
    }

    cout << flush;
}

void LinuxRender::render_other_board(Packet& pkt)
{
    string board_color;
    ...
    go(br, bc);
    for (int r = (BOARD_UPPER - 2); r < (BOARD_ROW - 2); ++r) {
        if ((br + r - (BOARD_UPPER - 2)) <= wr) continue;
        go(br + r - (BOARD_UPPER - 2), bc);
        mino_r = r - pos_r;
        for (int c = 0; c < BOARD_COL; ++c) {
            mino_c = c - pos_c;
            if (mino_r >= 0 && mino_r < MINO_SIZE && mino_c >= 0 && mino_c < MINO_SIZE)
                cout << ((m[mino_r][mino_c] != 0) ? block_color + "[]" : "\x1b[2C") << ansi_reset;
            else
                cout << "\x1b[2C";
        }
    }
 ...
 * @param 현재 점수
 */
void LinuxRender::render_score(int score)
{
    int wr = 13, wc = 32;
 ...
 * @param 현재 레벨
 */
void LinuxRender::render_level(int level)
{
    int wr = 16, wc = 32;
