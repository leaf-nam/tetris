#include "render/web_multi_renderer.hpp"
...
WebMultiRenderer::WebMultiRenderer(Setting* a1, ConsoleRenderer a2, ColorPicker a3,
...
void WebMultiRenderer::render_timer(int totalSec)
{
    int min = (totalSec / 60);
    int sec = totalSec % 60;
...
void WebMultiRenderer::render_clear() { console_renderer.clear(); }
void WebMultiRenderer::render_next_block(const int* tetrominoArray)
{
...
void WebMultiRenderer::render_hold(const Tetromino& tetromino)
{
    block_renderer.render_mino_pattern({5, 11}, tetromino, Color::PANEL);
}

void WebMultiRenderer::render_background()
{
    fflush(stdout);
...
void WebMultiRenderer::render_score(int score)
{
...
void WebMultiRenderer::render_level(int level)
{
...
void WebMultiRenderer::render_board(const Board& board, const Tetromino& tetromino)
{
...
void WebMultiRenderer::render_other_board(Packet& pkt)
{
...
void WebMultiRenderer::render_ip_recv()
{
    console_renderer.set_cursor(0, 0); // 적절한 위치로 수정 필요
    cout << "대전 상대의 IP를 입력하세요: ";
}

void WebMultiRenderer::render_char(char c) { cout << c; }

WebMultiRenderer::~WebMultiRenderer() {}
