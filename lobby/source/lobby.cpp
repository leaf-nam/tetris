  ...
  while (true) {
    if (lobby_input_handler->scan(s, sizeof(s), 0) == 1) {
      if (strcmp(s, "q") == 0) {
        ...
        lobby_network->send_udp(my_id, client_ip_address,
                                client_ip_address.size(), 0, 0, 0, 0, 1,
                                broadcast_ip);
        is_game_start = false;
        break;
      } else {
    ...
    if (std::chrono::steady_clock::now() - base_time >=
        std::chrono::milliseconds(5000)) {
      base_time = std::chrono::steady_clock::now();
      lobby_network->send_udp(my_id, client_ip_address,
                              client_ip_address.size(), 0, 0, 1, 0, 0,
                              broadcast_ip);
    }

    memset(ip, 16, sizeof(ip));
    user_data received_data;
    if (lobby_network->recv_udp(received_data, ip) == false)
      continue;

    if (index >= 4 || (received_data.is_enter == true &&
                       client_ip_address.find(std::string(received_data.id)) !=
                           client_ip_address.end()))
      lobby_network->send_udp(my_id, client_ip_address,
                              client_ip_address.size(), 1, 0, 0, 0, 0,
                              broadcast_ip);
    else {
      ...
      lobby_renderer->render_server_view_room(my_id, client_ip_address);
      lobby_network->send_multi_udp(my_id, client_ip_address,
                                    client_ip_address.size(), 0, 0, 0, 1, 0,
                                    client_ip_address);
    }
  }
  ...
  while (true) {
    if (lobby_input_handler->scan(s, sizeof(s), 0) == 1) {
      if (is_in_room == false && strcmp(s, "q") == 0) {
        ...
        if (server_ip_address.find(std::string(s)) == server_ip_address.end())
          continue;
    ...
    if (received_data.is_broadcast &&
      ...
      if (is_in_room == false ||
          (is_in_room == true &&
           strcmp(selected_server_ip_address, room_ip) == 0))
        lobby_renderer->render_view_enter_room(server_ip_address);
    } else if (received_data.is_update) {
      memset(selected_server_ip_address, 0, sizeof(selected_server_ip_address));
      snprintf(selected_server_ip_address, sizeof(selected_server_ip_address),
               "%s", room_ip);
      client_ip_address.clear();
...
const char *Lobby::get_server_ip_address() {
